...
if player_x < 0:
    player_x = chest_pos[0] + 1
player = Player(player_x, player_y, len(maze.potions))
...
print(f"Nouvelle position joueur: ({player.grid_x}, {player.grid_y})")

# Vérifier si le coffre est toujours présent dans les items
from entities_new import create_items_from_maze
items = create_items_from_maze(maze)
chest_items = [i for i in items if i.type == ItemType.CHEST]
print(f"Nombre de coffres dans items: {len(chest_items)}")